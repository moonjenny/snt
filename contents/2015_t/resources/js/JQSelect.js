...
*/
		...
		function showPopup() {
			...
			// dropdown
			var dropdown = $(dropdownTemplate).css('min-width', options.triggerWidth);
			...
			// list
			var list = dropdown.find('.jq-select-list');
			if (options.group) { // group
				...
				if (!$.isEmptyObject(options.data)) {
					for (var groupName in options.data) {
						...
						if (options.multi) {
							_self._value[groupName] && options.data[groupName]
							&& _self._value[groupName].length == options.data[groupName].length
						...
						options.data[groupName].forEach(function (item) {

							if (isPlainArrayData(options.data[groupName])) {
				...
				if ($.isArray(options.data)) {
					options.data.forEach(function (item) {
						...
						if (isPlainArrayData(options.data)) {
		...
		function bindFilterEvents() {
			...
			wrapper.find('.jq-select-filter').bind('input', function (e) {
				...
				if (text == '') {
					wrapper.find('.jq-select-group').removeClass('hidden');
					wrapper.find('.jq-select-item').removeClass('hidden');
					return;
				}

				wrapper.find('.jq-select-item').each(function () {
					if ($(this).attr('data-label').indexOf(text) == -1) {
				...
				wrapper.find('.jq-select-group').each(function () {
					...
					$(this).find('.jq-select-item').each(function () {
		...
		function bindSelectEvents() {
			...
			wrapper.find('.jq-select-select-all').change(function (e) {
				...
				//
				// 	if (_self.isPlainArrayData) {
				// 		selectedItems.push($(this).attr('data-value'));
				// 	} else {
				// 		var selectedItem = {};
				// 		selectedItem[options.valueField] = $(this).attr('data-value');
				// 		selectedItem[options.displayField] = $(this).attr('data-label');
			...
			wrapper.find('.jq-select-group-title').change(function (e) {
				...
				// $(this).parents('.jq-select-group').find('.jq-select-item').each(function () {
				//
				// 	$(this).children('input[type="checkbox"]').prop('checked', checked);
				//
				// 	var selectedItem = {};
