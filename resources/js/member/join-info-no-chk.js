...
$(function() {
	...
	*/
	...
	$('#joinButton').on('click', function() {
		...
		// 생년월일 셋팅
		//var bDayYear = jQuery("#brithdayYear").val();
		//var bDayMonth = jQuery("#brithdayMonth").val();
		//var bDayDay = jQuery("#brithdayDay").val();
		...
		var list = $('input[name="interestCategorys[]"]:checked');
		for(var i = 0; i < list.length; i++){
			if (i > 0) {
				interestCategory += ',';
			}
			interestCategory += list[i].value;
	    }
		$("input#interestCategory").val(interestCategory);
		if (_debug) console.log("interestCategory : " + interestCategory);
		*/
...
function checkEmNo() {
	...
	var url = "/member/em-no-check?emNo="+emNo+"&emName="+custName;
//	if (_debug) console.log('url : ' + url);
	commonAjax(url, null, "json", "GET")
		.done(function(data, textStatus, jqXHR) {
			if (_debug) console.log(data.code + ' : ' + data.message);
...
function validName(focus) {
	
	//$("#errorName").html("");
	//$("#errorName").css("display","none");
	var name = $('#name').val();
	var regTypeName = /^[가-힣]{2,5}$/;
	if(name == ""){
		$("#errorName").html("이름을 입력하세요.");
		$("#errorName").css("display","block");
		$("input#name").focus();
		return false;
	}else if(!regTypeName.test(name)){
		$("#errorName").html("이름은 2자~5자 이내의 한글로 올바르게 입력해 주세요.");
		$("#errorName").css("display","block");
		$("input#name").focus();
		return false;
	}else{
...
function hpChange(flag) {
	//	$(".phone_area3").css("display","none");
	//	$("#messageHp").removeClass('emp3');
	//	$("#messageHp").html("인증번호를 입력해 주세요.");
	//	$("#messageHp").css("display","none");
	//	authSms.init();
	var hpDdd = jQuery("#hpDdd");
	var hp2 = jQuery("#hp2");
	var hp3 = jQuery("#hp3");
    jQuery('#messageHp').css("display","none");
    jQuery('#messageHp').html('');
 
    if (comUtil.isNotEmpty(hpDdd.val())) {
        if (comUtil.isEmpty(hp2.val())) {
            jQuery('#messageHp').html('휴대전화번호를 입력해 주세요.');
            jQuery("#messageHp").css("display","block");
            if(flag){
            	hp2.focus();
            }
            return false;
        }else if(comUtil.isEmpty(hp3.val())) {
            jQuery('#messageHp').html('휴대전화번호를 입력해 주세요.');
            jQuery("#messageHp").css("display","block");
            if(flag){
            	hp3.focus();
    ...
    if (comUtil.isNotEmpty(hp2.val())) {
        if (comUtil.isEmpty(hp3.val())) {
            jQuery('#messageHp').html('휴대전화번호를 입력해 주세요.');
            jQuery("#messageHp").css("display","block");
            if(flag){
            	hp3.focus();
    ...
    if (comUtil.isNotEmpty(hpDdd.val()) && comUtil.isNotEmpty(hp2.val()) && comUtil.isNotEmpty(hp3.val())) {
        var hp = hpDdd.val() + "-" + hp2.val() + "-" + hp3.val();
        if (!comUtil.chkPhone(hp)) {
            jQuery('#messageHp').html('올바른 휴대전화 번호를 입력해 주세요.');
            jQuery("#messageHp").css("display","block");
            if(flag){
            	hp2.focus();
            }
