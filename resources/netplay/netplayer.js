...
//    });
    ...
    //부모 이벤트를 호출 하지 않도록 방지    
    jQuery('.func1').on('touchend', function(e) {
    	e.stopPropagation();
    });
    jQuery('.func1_on').on('touchend', function(e) {
    	e.stopPropagation();
    });
    jQuery('.func2').on('touchend', function(e) {
    	e.stopPropagation();
    });
    jQuery('.func3').on('touchend', function(e) {
    	e.stopPropagation();
    });
    jQuery('.func4').on('touchend', function(e) {
    	e.stopPropagation();
    });
    jQuery('.func_back').on('touchend', function(e) {
    	e.stopPropagation();
    });
    jQuery('.btn_buy').on('touchend', function(e) {
    	e.stopPropagation();
    });
    jQuery('.func6').on('touchend', function(e) {
    	e.stopPropagation();
    });
    jQuery('.btn_play').on('touchend', function(e) {
    	e.stopPropagation();
    });
    jQuery('.btn_stop').on('touchend', function(e) {
    	e.stopPropagation();
    });
    
    
    jQuery('.func1').click(function() {
        window.location=APP_SCHEMA + "mute?true";
        gMute = true;
        jQuery('.func1').css('display', 'none');
        jQuery('.func1_on').css('display', 'block');
    });
    
    jQuery('.func1_on').click(function() {
        window.location=APP_SCHEMA + "mute?false";
        gMute = false;
        jQuery('.func1').css('display', 'block');
        jQuery('.func1_on').css('display', 'none');
    });
    ...
    jQuery('.func_back').click(function() {
    	window.location=APP_SCHEMA + "close?true";
    	jQuery('.btn_play').hide();
    	jQuery('.btn_stop').show();
    });
    
    jQuery('.btn_buy').click(function() {
...
function setParam(param){
	playVideoParam = jQuery.parseJSON(param);
	jQuery('#title_text').html(playVideoParam.title);
	jQuery('#title_price').html("");
	if(playVideoParam.price!=undefined){
		//<strong>96,000</strong>원
		jQuery('#title_price').html("<strong>"+playVideoParam.price+"</strong>원");
	}
	gLinkUrl = playVideoParam.linkUrl;
	
	var endDate = new Date(playVideoParam.endDate);
    var curDate = new Date();
	leftTime = (endDate - curDate) / 1000;
	
	jQuery(this).stopTime("netplayerLiveTimer");
	jQuery(this).everyTime(1000, "netplayerLiveTimer", timerFunc);
	
	function timerFunc(){
        if (leftTime <= 0) {
        	jQuery(".onair_time").html('ON AIR<span></span>남은 시간 : 00:00:00');
        } else {
            ...
            if(Math.floor(leftHour/24) > 0){
                ...
                var timeText = leftDay + "d" +
                (leftHour < 10 ? "0" : "") + leftHour + ":" +
                (leftMin < 10 ? "0" : "") + leftMin;
                jQuery(".onair_time").html('ON AIR<span></span>남은 시간 : '+timeText);
            }else{
            	var timeText = (leftHour < 10 ? "0" : "") + leftHour + ":" +
                (leftMin < 10 ? "0" : "") + leftMin + ":" +
...
function setMode(val){
	...
	case "small":
		...
		jQuery('.func1').hide();
		jQuery('.func1_on').hide();
		redrawLayoutWithSafeAreaInset();  //추가
		break;
	case "default":
		jQuery('.play_wrap').attr('class','play_wrap'); //2016-06-21
		jQuery('.timeline_area').hide();
		if(playVideoParam.live == "true" && gLinkUrl){
			jQuery('.btn_buy').show();
		}else{
			jQuery('.btn_buy').hide();
		}
		if(gMute){
			jQuery('.func1').hide();
			jQuery('.func1_on').show();
		}else{
			jQuery('.func1').show();
			jQuery('.func1_on').hide();
		}
		redrawLayoutWithSafeAreaInset();  //추가
		//jQuery('.func1').css('opacity', '1');
		//jQuery('.func1_on').css('opacity', '1');
		break;
	case "full":
		jQuery('.play_wrap').attr('class','play_wrap full'); //2016-06-21
		if(playVideoParam.live == "true" ){
			gLinkUrl=="" ? jQuery('.btn_buy').hide() : jQuery('.btn_buy').show();
		}else{
			jQuery('.onair_time').hide();
			jQuery('.btn_buy').hide();
...
//iPhone X와 같은 Safe area를 사용하는 기기의 경우 처리
function redrawLayoutWithSafeAreaInset(){
    if (deviceType != 'iOS') {
        return;
    }

    var screenWidth = jQuery(this).width();
    var screenHeight = jQuery(this).height();
    var isFullVideo = (screenWidth > screenHeight) ? true : false;
    var applySafeArea = false;

    if (isFullVideo) {
        //풀스크린이면서 종횡비가 2 이상인 경우에는 Safe area inset 마진을 화면 구성 요소에 적용하여야 함.
        //iPhone X의 경우 2.16이 산출되며, 일반 16:9 기기의 경우 1.77이, 4:3 기기의 경우에는 1.33 가량이 산출된다.
    ...
    if (applySafeArea) { //Safe area 적용 대상 - iPhone X의 전체화면
        jQuery('.func6').css('right','59px');
        jQuery('.func2').css('left','59px');
        jQuery('.func4').css('left','114px');
        jQuery('.func3').css('left','114px');
        jQuery('.func1_on').css('left','169px');
        jQuery('.func1').css('left','169px');
        jQuery('.btn_buy').css('right','59px');
        jQuery('#title_price').css('right', '69px');
        jQuery('.func_back').css('margin-left', '59px');
    } else { //Safe area 미적용 대상 - iPhone X의 일반화면 또는 기타 기종
        jQuery('.func6').css('right','15px');
        jQuery('.func6').css('right','15px');
        jQuery('.func2').css('left','15px');
        jQuery('.func4').css('left','70px');
        jQuery('.func3').css('left','70px');
        jQuery('.func1_on').css('left','125px');
        jQuery('.func1').css('left','125px');
        jQuery('.btn_buy').css('right','15px');
        jQuery('#title_price').css('right', '25px');
        jQuery('.func_back').css('margin-left', '15px');
    }
}
